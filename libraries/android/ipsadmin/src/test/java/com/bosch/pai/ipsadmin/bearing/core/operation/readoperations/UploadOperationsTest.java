...
public class UploadOperationsTest {
    ...
    @Test(timeout = 1000*10)
    public void uploadDataAsynchronousResponseServerCallTest(){
        String requestId = String.valueOf(UUID.randomUUID());
        Sender sender = null;
        RequestUploadEvent requestUploadEvent = new RequestUploadEvent(requestId, EventType.CAPTURE_DATA_EVENT, sender);
        requestUploadEvent.setLocationName("location");
        requestUploadEvent.setFetchRequest(RequestUploadEvent.ServerFetch.SITE_UPLOAD);
        requestUploadEvent.setSiteName("siteName");
        requestUploadEvent.setRequestID(String.valueOf(UUID.randomUUID()));
        requestUploadEvent.setApproach(BearingConfiguration.Approach.THRESHOLDING);
        getUploadOperations.uploadDataAsynchronousResponseServerCall(requestUploadEvent);
    }
