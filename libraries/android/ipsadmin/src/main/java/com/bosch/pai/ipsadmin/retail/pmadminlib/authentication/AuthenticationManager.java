...
public final class AuthenticationManager implements CommsListener {
    ...
    public void changePassword(Context context, String oldPassword, String newPassword, AuthenticationCallback callback) {
        this.authenticationCallback = callback;
        initComms();
        final RequestObject requestObject = new RequestObject(RequestObject.RequestType.PUT, this.currentBaseUrl,
                "/registration/companies/" + this.currentCompanyId + "/users/" + this.currentUserId);
        if(Util.isHttpsURL(this.currentBaseUrl)) {
            cert = Util.getCertificate(context);
            requestObject.setCertFileStream(cert);
        }
        final PasswordChangeModel passwordChangeModel = new PasswordChangeModel();
        passwordChangeModel.setOldPassword(oldPassword);
        passwordChangeModel.setNewPassword(newPassword);
        passwordChangeModel.setCredentialsExpiry(30);   // Default expiry period for 30 days
        requestObject.setMessageBody(new GsonBuilder().create().toJson(passwordChangeModel));
        this.commsManager.processRequest(requestObject, new CommsListener() {
            @Override
            ...
            public void onFailure(int statusCode, String errMessage) {
                handler.sendMessage(handler.obtainMessage(FAILURE, errMessage));
            }
        });
    }
