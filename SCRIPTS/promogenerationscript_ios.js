...
db.getCollection('SSL_valid_promo_details_ios').remove({});
...
/* #3 Convert MEN�S FORMAL to MENS FORMAL and MEN�S LEATHER and luggage to MENS LEATHER and luggage */    
...
db.SSL_LOCATION_BAY_MAP_IOS.aggregate([
{
	$addFields: {
		locationCategorys: {
			$cond: [{
...
{
	"$out": "SSL_valid_promo_details_ios"
}],
{
	explain: false,
	cursor: {
...
db.getCollection('SSL_valid_promo_details_ios').aggregate([
{
   $addFields: {
      "PROMO_START_DATE": {
         $dateFromString: {
